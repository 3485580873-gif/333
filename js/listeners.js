function setupEventListeners() {
    try {
        ...
        initMoodListeners();
        initDecisionModule();function setupEventListeners() {
    try {function setupEventListeners() {
    try {
        initCoreListeners();
        initModalListeners();
        initChatActionListeners();
...
// 分钟数格式化：>=60 显示"X小时"或"X小时Y分钟"，否则显示"X分钟"
function formatMomentsAutoPostMinutes(mins) {
    mins = Number(mins) || 0;
...
const updateMomentsAutoPostUI = () => {
    ...
    if (momentsAutoPostMinSlider) {
        const minVal = settings.momentsAutoPostMinMinutes || 30;
        momentsAutoPostMinSlider.value = minVal;
        if (momentsAutoPostMinValue) momentsAutoPostMinValue.textContent = formatMomentsAutoPostMinutes(minVal);
    }
    if (momentsAutoPostMaxSlider) {
        const maxVal = settings.momentsAutoPostMaxMinutes || 600;
        momentsAutoPostMaxSlider.value = maxVal;
        if (momentsAutoPostMaxValue) momentsAutoPostMaxValue.textContent = formatMomentsAutoPostMinutes(maxVal);
    }
...
if (momentsAutoPostMinSlider) {
    momentsAutoPostMinSlider.addEventListener('input', (e) => {
        ...
        const maxAllowed = settings.momentsAutoPostMaxMinutes || 600;
        if (val > maxAllowed) {
            // 只在确实超出时才写回，且用 requestAnimationFrame 避免同帧内的布局抖动
            e.target.value = maxAllowed;
            val = maxAllowed;
        }
        settings.momentsAutoPostMinMinutes = val;
        if (momentsAutoPostMinValue) momentsAutoPostMinValue.textContent = formatMomentsAutoPostMinutes(val);
    });
    momentsAutoPostMinSlider.addEventListener('change', () => {
        throttledSaveData();
        if (typeof manageMomentsAutoPostTimer === 'function') manageMomentsAutoPostTimer();
    });
}
...
if (momentsAutoPostMaxSlider) {
    momentsAutoPostMaxSlider.addEventListener('input', (e) => {
        let val = parseInt(e.target.value, 10);
        const minAllowed = settings.momentsAutoPostMinMinutes || 30;
        if (val < minAllowed) {
            // 只在确实低于最小值时才写回
            e.target.value = minAllowed;
            val = minAllowed;
        }
        settings.momentsAutoPostMaxMinutes = val;
        if (momentsAutoPostMaxValue) momentsAutoPostMaxValue.textContent = formatMomentsAutoPostMinutes(val);
    });
    momentsAutoPostMaxSlider.addEventListener('change', () => {
        throttledSaveData();
        if (typeof manageMomentsAutoPostTimer === 'function') manageMomentsAutoPostTimer();
    });
}
...
if (momentsAutoPostMinSlider) {
    momentsAutoPostMinSlider.addEventListener('input', (e) => {
        let val = parseInt(e.target.value, 10);
        if (val > (settings.momentsAutoPostMaxMinutes || 600)) {
            val = settings.momentsAutoPostMaxMinutes || 600;
            momentsAutoPostMinSlider.value = val;
        }
        settings.momentsAutoPostMinMinutes = val;
        if (momentsAutoPostMinValue) momentsAutoPostMinValue.textContent = formatMomentsAutoPostMinutes(val);
    });
...
if (momentsAutoPostMaxSlider) {
    momentsAutoPostMaxSlider.addEventListener('input', (e) => {
        let val = parseInt(e.target.value, 10);
        if (val < (settings.momentsAutoPostMinMinutes || 30)) {
            val = settings.momentsAutoPostMinMinutes || 30;
            momentsAutoPostMaxSlider.value = val;
        }
        settings.momentsAutoPostMaxMinutes = val;
        if (momentsAutoPostMaxValue) momentsAutoPostMaxValue.textContent = formatMomentsAutoPostMinutes(val);
    });
...
if (momentsCardCountMinSlider) {
    momentsCardCountMinSlider.addEventListener('input', (e) => {
        let val = parseInt(e.target.value, 10);
        if (val > (settings.momentsCardCountMax || 5)) {
            val = settings.momentsCardCountMax || 5;
            momentsCardCountMinSlider.value = val;
        }
        settings.momentsCardCountMin = val;
        if (momentsCardCountMinValue) momentsCardCountMinValue.textContent = val + '条';
    });
...
if (momentsCardCountMaxSlider) {
    momentsCardCountMaxSlider.addEventListener('input', (e) => {
        let val = parseInt(e.target.value, 10);
        if (val < (settings.momentsCardCountMin || 3)) {
            val = settings.momentsCardCountMin || 3;
            momentsCardCountMaxSlider.value = val;
        }
        settings.momentsCardCountMax = val;
        if (momentsCardCountMaxValue) momentsCardCountMaxValue.textContent = val + '条';
    });
