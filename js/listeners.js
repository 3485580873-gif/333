...
// 触屏设备上，用「点击 vs 滑动」的判定来代替 :hover 弹出操作菜单，
// 避免在聊天列表里轻轻上下滑动时，手指经过消息气泡就被当成 hover 触发菜单。
function initMobileMessageActionsToggle() {
    const container = DOMElements.chatContainer;
    if (!container) return;

    const MOVE_THRESHOLD = 10; // 超过这个像素距离就判定为滑动，不弹菜单
    ...
    container.addEventListener('touchend', (e) => {
        if (touchMoved) return; // 是滚动/滑动，不弹出菜单

        // 点击的是菜单里的按钮（回复/收藏/撤回/删除等），交给原有点击逻辑处理
        if (e.target.closest('.meta-action-btn') || e.target.closest('.favorite-action-btn')) return;
        // 语音字卡（播放按钮/波形/转文字按钮）本身就是可点击控件，点它只应该播放/转文字，不应该弹出操作菜单
        if (e.target.closest('.voice-message-bubble') || e.target.closest('.voice-transcript')) return;
        // 红包和问卷卡片点击只触发自身交互，不弹出操作菜单（和语音字卡保持一致）
        if (e.target.closest('.red-packet-card') || e.target.closest('.dq-card-bubble') || e.target.closest('[data-dq-id]')) return;

        const contentWrapper = e.target.closest('.message-content-wrapper');

        // 先收起其他已经展开的菜单
        container.querySelectorAll('.message-content-wrapper.show-actions').forEach(el => {
            if (el !== contentWrapper) el.classList.remove('show-actions');
        });

        if (contentWrapper) {
            contentWrapper.classList.toggle('show-actions');
        }
    });

    // 点击聊天区域以外的地方，收起已展开的操作菜单
    document.addEventListener('touchstart', (e) => {
        if (!container.contains(e.target)) {
            container.querySelectorAll('.message-content-wrapper.show-actions').forEach(el => el.classList.remove('show-actions'));
        }
    }, { passive: true });
}

function initChatActionListeners() {
            initMobileMessageActionsToggle();
...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    // 同步朋友圈自动发布开关（之前缺失导致刷新后开关重置）
    if (typeof updateMomentsAutoPostUI === 'function') updateMomentsAutoPostUI();
...
const momentsAutoPostMinSlider = document.getElementById('moments-auto-post-min-slider');
const momentsAutoPostMinValue = document.getElementById('moments-auto-post-min-value');
const momentsAutoPostMinUnit = document.getElementById('moments-auto-post-min-unit');
const momentsAutoPostMaxSlider = document.getElementById('moments-auto-post-max-slider');
const momentsAutoPostMaxValue = document.getElementById('moments-auto-post-max-value');
const momentsAutoPostMaxUnit = document.getElementById('moments-auto-post-max-unit');

// 单位标签映射（与时空来信一致）
const momentsUnitLabelMap = { minutes: '分钟', hours: '小时', days: '天' };
const momentsCardCountMinSlider = document.getElementById('moments-card-count-min-slider');
const momentsCardCountMinValue = document.getElementById('moments-card-count-min-value');
const momentsCardCountMaxSlider = document.getElementById('moments-card-count-max-slider');
const momentsCardCountMaxValue = document.getElementById('moments-card-count-max-value');
const momentsSourceMainToggle = document.getElementById('moments-source-main-toggle');
const momentsSourceKaomojiToggle = document.getElementById('moments-source-kaomoji-toggle');
const momentsSourceEmojiToggle = document.getElementById('moments-source-emoji-toggle');
const momentsSourceStickerToggle = document.getElementById('moments-source-sticker-toggle');

// 单位+数值格式化（与时空来信一致）
function formatMomentsAutoPostVal(val, unit) {
    return (val || 1) + (momentsUnitLabelMap[unit] || '分钟');
}

const updateMomentsAutoPostUI = () => {
    if (momentsAutoPostToggle) momentsAutoPostToggle.classList.toggle('active', !!settings.momentsAutoPostEnabled);
    if (momentsAutoPostSettings) momentsAutoPostSettings.style.display = settings.momentsAutoPostEnabled ? 'block' : 'none';
    const minVal = settings.momentsAutoPostMinVal != null ? settings.momentsAutoPostMinVal : 30;
    const minUnit = settings.momentsAutoPostMinUnit || 'minutes';
    const maxVal = settings.momentsAutoPostMaxVal != null ? settings.momentsAutoPostMaxVal : 10;
    const maxUnit = settings.momentsAutoPostMaxUnit || 'hours';
    if (momentsAutoPostMinSlider) momentsAutoPostMinSlider.value = Math.min(minVal, 10);
    if (momentsAutoPostMinUnit) momentsAutoPostMinUnit.value = minUnit;
    if (momentsAutoPostMinValue) momentsAutoPostMinValue.textContent = formatMomentsAutoPostVal(minVal, minUnit);
    if (momentsAutoPostMaxSlider) momentsAutoPostMaxSlider.value = Math.min(maxVal, 10);
    if (momentsAutoPostMaxUnit) momentsAutoPostMaxUnit.value = maxUnit;
    if (momentsAutoPostMaxValue) momentsAutoPostMaxValue.textContent = formatMomentsAutoPostVal(maxVal, maxUnit);
    if (momentsCardCountMinSlider) {
        const v = settings.momentsCardCountMin || 3;
        momentsCardCountMinSlider.value = v;
        if (momentsCardCountMinValue) momentsCardCountMinValue.textContent = v + '条';
    }
    if (momentsCardCountMaxSlider) {
        const v = settings.momentsCardCountMax || 5;
        momentsCardCountMaxSlider.value = v;
        if (momentsCardCountMaxValue) momentsCardCountMaxValue.textContent = v + '条';
    }
    if (momentsSourceMainToggle) momentsSourceMainToggle.classList.toggle('active', settings.momentsSourceMain !== false);
    if (momentsSourceKaomojiToggle) momentsSourceKaomojiToggle.classList.toggle('active', settings.momentsSourceKaomoji !== false);
    if (momentsSourceEmojiToggle) momentsSourceEmojiToggle.classList.toggle('active', settings.momentsSourceEmoji !== false);
    if (momentsSourceStickerToggle) momentsSourceStickerToggle.classList.toggle('active', settings.momentsSourceSticker !== false);
};
updateMomentsAutoPostUI();

if (momentsAutoPostToggle) {
    momentsAutoPostToggle.addEventListener('click', () => {
        settings.momentsAutoPostEnabled = !settings.momentsAutoPostEnabled;
        updateMomentsAutoPostUI();
        if (typeof manageMomentsAutoPostTimer === 'function') manageMomentsAutoPostTimer();
        throttledSaveData();
        showNotification(`对方主动发朋友圈已${settings.momentsAutoPostEnabled ? '开启' : '关闭'}`, 'success');
    });
}

if (momentsAutoPostMinSlider) {
    momentsAutoPostMinSlider.addEventListener('input', (e) => {
        const val = parseInt(e.target.value, 10);
        settings.momentsAutoPostMinVal = val;
        if (momentsAutoPostMinValue) momentsAutoPostMinValue.textContent = formatMomentsAutoPostVal(val, settings.momentsAutoPostMinUnit || 'minutes');
    });
    momentsAutoPostMinSlider.addEventListener('change', () => { throttledSaveData(); if (typeof manageMomentsAutoPostTimer === 'function') manageMomentsAutoPostTimer(true); });
}
if (momentsAutoPostMinUnit) {
    momentsAutoPostMinUnit.addEventListener('change', (e) => {
        settings.momentsAutoPostMinUnit = e.target.value;
        if (momentsAutoPostMinValue) momentsAutoPostMinValue.textContent = formatMomentsAutoPostVal(settings.momentsAutoPostMinVal || 30, e.target.value);
        throttledSaveData(); if (typeof manageMomentsAutoPostTimer === 'function') manageMomentsAutoPostTimer(true);
    });
}

if (momentsAutoPostMaxSlider) {
    momentsAutoPostMaxSlider.addEventListener('input', (e) => {
        const val = parseInt(e.target.value, 10);
        settings.momentsAutoPostMaxVal = val;
        if (momentsAutoPostMaxValue) momentsAutoPostMaxValue.textContent = formatMomentsAutoPostVal(val, settings.momentsAutoPostMaxUnit || 'hours');
    });
    momentsAutoPostMaxSlider.addEventListener('change', () => { throttledSaveData(); if (typeof manageMomentsAutoPostTimer === 'function') manageMomentsAutoPostTimer(true); });
}
if (momentsAutoPostMaxUnit) {
    momentsAutoPostMaxUnit.addEventListener('change', (e) => {
        settings.momentsAutoPostMaxUnit = e.target.value;
        if (momentsAutoPostMaxValue) momentsAutoPostMaxValue.textContent = formatMomentsAutoPostVal(settings.momentsAutoPostMaxVal || 10, e.target.value);
        throttledSaveData(); if (typeof manageMomentsAutoPostTimer === 'function') manageMomentsAutoPostTimer(true);
    });
}

if (momentsCardCountMinSlider) {
    momentsCardCountMinSlider.addEventListener('input', (e) => {
        let val = parseInt(e.target.value, 10);
        const maxAllowed = settings.momentsCardCountMax || 5;
        if (val > maxAllowed) {
            e.target.value = maxAllowed;
            val = maxAllowed;
        }
        settings.momentsCardCountMin = val;
        if (momentsCardCountMinValue) momentsCardCountMinValue.textContent = val + '条';
    });
    momentsCardCountMinSlider.addEventListener('change', () => { throttledSaveData(); });
}

if (momentsCardCountMaxSlider) {
    momentsCardCountMaxSlider.addEventListener('input', (e) => {
        let val = parseInt(e.target.value, 10);
        const minAllowed = settings.momentsCardCountMin || 3;
        if (val < minAllowed) {
            e.target.value = minAllowed;
            val = minAllowed;
        }
        settings.momentsCardCountMax = val;
        if (momentsCardCountMaxValue) momentsCardCountMaxValue.textContent = val + '条';
    });
    momentsCardCountMaxSlider.addEventListener('change', () => { throttledSaveData(); });
}

// 内容来源开关：主字卡 / 颜文字 / emoji / 表情库，各自独立开关，互不影响其它开关的渲染逻辑
const momentsSourceToggleMap = {
    momentsSourceMainToggle: 'momentsSourceMain',
    momentsSourceKaomojiToggle: 'momentsSourceKaomoji',
    momentsSourceEmojiToggle: 'momentsSourceEmoji',
    momentsSourceStickerToggle: 'momentsSourceSticker'
};
const momentsSourceToggleEls = {
    momentsSourceMainToggle, momentsSourceKaomojiToggle, momentsSourceEmojiToggle, momentsSourceStickerToggle
};
const momentsSourceToggleNames = {
    momentsSourceMainToggle: '主字卡',
    momentsSourceKaomojiToggle: '颜文字',
    momentsSourceEmojiToggle: 'emoji',
    momentsSourceStickerToggle: '表情库'
};
Object.keys(momentsSourceToggleMap).forEach((key) => {
    const el = momentsSourceToggleEls[key];
    const prop = momentsSourceToggleMap[key];
    if (!el) return;
    el.addEventListener('click', () => {
        const newVal = !(settings[prop] !== false);
        // 至少保留一个内容来源，不能全部关闭
        const allProps = Object.values(momentsSourceToggleMap);
        const otherEnabled = allProps.some(p => p !== prop && settings[p] !== false);
        if (!newVal && !otherEnabled) {
            showNotification('至少要保留一个内容来源哦', 'info', 2000);
            return;
        }
        settings[prop] = newVal;
        el.classList.toggle('active', newVal);
        throttledSaveData();
        showNotification(`${momentsSourceToggleNames[key]}已${newVal ? '开启' : '关闭'}`, 'success', 1200);
    });
});
...
if (envelopeReplyMaxSentencesSlider) {
    ...
    if (envelopeEntryBtn) {
        envelopeEntryBtn.addEventListener('click', async () => {
            ...
            // 重置tab状态（兼容两种版本的信箱HTML）
            const tabOutbox = document.getElementById('env-tab-outbox');
            const tabInbox = document.getElementById('env-tab-inbox');
            const tabSpacetime = document.getElementById('env-tab-spacetime');
            if (tabOutbox) tabOutbox.classList.add('active');
            if (tabInbox) tabInbox.classList.remove('active');
            if (tabSpacetime) tabSpacetime.classList.remove('active');
            ...
            const spacetimeSection = document.getElementById('env-spacetime-section');
            if (outboxSection) outboxSection.style.display = 'block';
            if (inboxSection) inboxSection.style.display = 'none';
            if (spacetimeSection) spacetimeSection.style.display = 'none';
    ...
    // 小红书解析入口
    const xhsEntryBtn = document.getElementById('xhs-parser-function');
    if (xhsEntryBtn) {
        xhsEntryBtn.addEventListener('click', () => {
            hideModal(DOMElements.advancedModal.modal);
            if (typeof initXhsParser === 'function') initXhsParser();
            showModal(document.getElementById('xhs-parser-modal'));
        });
    }
    // 嵌入网页入口
    const webEmbedEntryBtn = document.getElementById('web-embed-function');
    if (webEmbedEntryBtn) {
        webEmbedEntryBtn.addEventListener('click', () => {
            hideModal(DOMElements.advancedModal.modal);
            showModal(document.getElementById('web-embed-modal'));
        });
    }
    // 梦向问卷入口
