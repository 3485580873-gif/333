...
(function() {
    ...
    var _manualStop = false; // 区分「用户主动关闭」还是「被系统/其他App抢占打断」
    ...
    function _createAudio() {
        ...
        _audio.addEventListener('pause', function(){
            _setUI(false);
            // 只要不是用户自己点开关关闭的，就是被系统/其他App（比如打开哔哩哔哩播放视频）抢占打断了，
            // 立刻尝试自动抢回来，不用等用户手动重新开一次
            if (!_manualStop && _get()) {
                _tryResume();
            }
        });
    ...
    // 统一的「尝试恢复播放」逻辑：立刻试一次，不行的话挂一个一次性的全局点击/触摸监听，
    // 只要你在app里点一下任何地方，就顺手把保活音频接回来，不用特意去开关那里重新点一次
    function _tryResume() {
        if (!_get()) return;
        var a = _createAudio();
        var p = a.play();
    ...
    function _stop() {
        ...
        _setUI(false);
        // 短暂延迟后复位标记，避免影响之后真正的意外中断判断
        setTimeout(function(){ _manualStop = false; }, 500);
    }

    // 供外部（如语音播放结束后）调用，重新唤起保活音频
    window._resumeKeepaliveAudio = function() {
    ...
    // iOS 从后台切回来、或者从 bfcache 恢复时，也顺手尝试恢复一次
    window.addEventListener('pageshow', function(){
        if (_get() && _audio && _audio.paused) _tryResume();
    });
    window.addEventListener('focus', function(){
        if (_get() && _audio && _audio.paused) _tryResume();
    });

    // 兜底看门狗：每 4 秒检查一次，如果开关是开着的但音频意外停了（比如被其他App抢占后系统又把资源还回来了），
    // 就自动接回来。很多时候等对方App自己不再播放了，脚本调用 play() 反而不需要新的用户手势也能成功。
    _watchdogTimer = setInterval(function(){
        if (_get() && _audio && _audio.paused && !_manualStop) {
            _tryResume();
        }
    }, 4000);
...
window.reopenDailyGreeting = function() {
    try {
        ...
        if (modal) {
            ...
            // 每次打开公告都滚回顶部
            var body = modal.querySelector('.dg-body');
...
window.tryShowDailyGreeting = function() {
    try {
        ...
        if (!todayMood || !todayMood.partner) {
            setTimeout(_showGreeting, 45000);
            return;
        }
        _showGreeting();
...
window.openHistoryJumpPanel = function() {
    var panel = document.getElementById('history-jump-panel');
    if (!panel) return;

    // 生成最近有消息的日期快捷按钮（最多7天）
    var shortcuts = document.getElementById('history-jump-shortcuts');
    if (shortcuts && typeof messages !== 'undefined' && messages.length) {
        var dateSet = {};
        ...
        var dates = Object.keys(dateSet).sort().reverse().slice(0, 7);
        shortcuts.innerHTML = dates.map(function(dateStr) {
            var d = new Date(dateStr + 'T12:00:00');
            var label = (d.getMonth()+1) + '月' + d.getDate() + '日';
            return '<button onclick="quickJumpToDate(\'' + dateStr + '\')" style="'
    ...
    // 设置日期范围限制
    var dateInput = document.getElementById('history-jump-date');
    if (dateInput && typeof messages !== 'undefined' && messages.length) {
        var first = messages[0];
        if (first && first.timestamp) {
            var fd = new Date(first.timestamp);
            dateInput.min = fd.getFullYear() + '-' +
...
window.closeHistoryJumpPanel = function() {
    var panel = document.getElementById('history-jump-panel');
    if (panel) panel.style.display = 'none';
};

window.quickJumpToDate = function(dateStr) {
    var inp = document.getElementById('history-jump-date');
    if (inp) inp.value = dateStr;
    window.doHistoryJump();
};

window.doHistoryJump = function() {
    var inp = document.getElementById('history-jump-date');
    if (!inp || !inp.value) return;
    var targetDate = new Date(inp.value + 'T00:00:00');
    window.closeHistoryJumpPanel();
    ...
    messages.forEach(function(m, i) {
        if (!m.timestamp) return;
        var diff = Math.abs(new Date(m.timestamp) - targetDate);
        if (diff < bestDiff) { bestDiff = diff; bestIdx = i; }
    });

    if (bestIdx < 0) {
        if (typeof showNotification === 'function') showNotification('该日期没有消息记录', 'info', 2000);
        return;
    }

    // 确保目标消息已渲染
    var totalMessages = messages.length;
    var needed = totalMessages - bestIdx;
    if (typeof displayedMessageCount !== 'undefined' && needed > displayedMessageCount) {
        window.displayedMessageCount = Math.min(totalMessages, needed + 20);
        if (typeof renderMessages === 'function') renderMessages(false);
    }
    ...
    setTimeout(function() {
        ...
        if (!container) return;
        var targetMsg = messages[bestIdx];
        if (!targetMsg) return;

        // 按 data-msg-id 定位 DOM 节点
        var msgId = targetMsg.id || targetMsg.msgId;
        var el = msgId ? container.querySelector('[data-msg-id="' + msgId + '"]') : null;

        // 按渲染顺序回退定位
        if (!el) {
            var all = container.querySelectorAll('.message-wrapper');
            var offset = Math.max(0, totalMessages - (window.displayedMessageCount || totalMessages));
            var renderedIdx = bestIdx - offset;
            if (renderedIdx >= 0 && all[renderedIdx]) el = all[renderedIdx];
        }
        ...
        var label = (d.getMonth()+1) + '月' + d.getDate() + '日';
        if (typeof showNotification === 'function') showNotification('已跳转到 ' + label, 'success', 1800);
    }, 200);
};

window.scrollToLatestMessage = function() {
    // 找聊天滚动容器
    var container = document.getElementById('chat-container')
        || (typeof DOMElements !== 'undefined' && DOMElements.chatContainer)
        || null;
    if (!container) return;

    // 强制滚动到底部（用 scrollTo 配合 auto 绕过 smooth CSS）
    try {
    ...
    setTimeout(function() {
        ...
        var btn = document.getElementById('back-to-latest-btn');
