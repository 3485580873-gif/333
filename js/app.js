document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // loadData 完成后 settings 已从 localforage 读回真实值，
        // 重新刷新所有依赖 settings 的 UI 开关/滑杆
        // （它们在 setupEventListeners 里首次调用时 settings 还是默认值，
        // 导致刷新或闪退后开关被重置为关闭状态）
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'hidden') {
                ...
                // 回到前台时：若有缓存的「发送失败」通知，立即弹出
                try {
                    const pending = sessionStorage.getItem('_send_failed_pending');
                    if (pending) {
                        sessionStorage.removeItem('_send_failed_pending');
                        setTimeout(() => {
                            const existing = document.getElementById('_send_failed_notification');
                            if (!existing && typeof window._showSendFailedNotificationGlobal === 'function') {
                                window._showSendFailedNotificationGlobal(pending);
                            }
                        }, 400);
                    }
                } catch(e) {}
