...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (isVoice) {
        ...
        if (voiceBubble) {
            ...
            // 提前创建并预加载音频，避免第一次点击时才开始创建/加载，导致点击后要等一下才有反应
            const ensureVoiceAudio = () => {
                ...
                voiceAudio.addEventListener('ended', () => {
                    ...
                    // 语音播放完毕后，恢复保活音频（iOS媒体会话被语音占用后需重新唤起）
                    try {
                        var ka = localStorage.getItem('keepaliveAudioEnabled') === 'true';
                        if (ka && window._resumeKeepaliveAudio) window._resumeKeepaliveAudio();
                    } catch(e) {}
                });
            };
            // 语音气泡刚渲染出来时就预加载，而不是等用户点了才加载
            ensureVoiceAudio();
