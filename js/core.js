    ...
    if (!shouldIgnore) {
        if (settings.typingIndicatorEnabled) {
            ...
            window._typingIndicatorAutoHideTimer = setTimeout(() => {
                _hideTypingNow();
                // 兜底超时：仅在 _pendingReplyTimer 尚未触发时才重发，避免与正常流程重复调用 simulateReply
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            for (let i = 0; i < replyCount; i++) {
                if (i === 0) {
                    // Bug 2 修复：用户已在 sendMessage 里等过一次 replyDelay，
                    // 这里只加短暂"打字感"延迟，避免两段 replyDelay 叠加导致总等待过长
                    delay = 800 + Math.random() * 1200;
                    // Bug 1 修复：用新的 delay 续期 AutoHideTimer，防止气泡在消息到达前被提前收走
                    if (window._typingIndicatorAutoHideTimer) clearTimeout(window._typingIndicatorAutoHideTimer);
                    window._typingIndicatorAutoHideTimer = setTimeout(function() {
                        ...
                        if (_tiW) { var _ti = _tiW.querySelector('.typing-indicator'); if (_ti) { _ti.classList.add('hiding'); setTimeout(function(){ _tiW.style.display='none'; _ti.classList.remove('hiding'); }, 240); } else { _tiW.style.display='none'; } }
                    }, delay + 3000);
                } else {
