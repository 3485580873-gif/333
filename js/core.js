            ...
            if (transBtn) {
                transBtn.addEventListener('click', function(e) {
                    ...
                    if (existing) {
                        ...
                        // 字卡旁播放按钮
                        const transcriptPlayBtn = document.createElement('button');
                        transcriptPlayBtn.className = 'voice-transcript-play-btn';
                        const syncTransIcon = () => {
                            transcriptPlayBtn.innerHTML = voiceBubble.classList.contains('voice-playing')
                                ? '<i class="fas fa-pause"></i>'
                                : '<i class="fas fa-play"></i>';
                        };
                        syncTransIcon();
                        transcriptPlayBtn.addEventListener('click', function(ev) {
                            ev.stopPropagation();
                            voiceBubble.dispatchEvent(new MouseEvent('click', { bubbles: false }));
                        });
                        const observer = new MutationObserver(syncTransIcon);
                        observer.observe(voiceBubble, { attributes: true, attributeFilter: ['class'] });
                        transcriptDiv._observer = observer;

                        const textSpan = document.createElement('span');
                        textSpan.className = 'voice-transcript-text';
                        textSpan.textContent = voiceText || '';

                        transcriptDiv.appendChild(transcriptPlayBtn);
