...
function manageMomentsAutoPostTimer(forceRecalculate) {
    ...
    // 用户主动修改了时间区间（滑块/单位）时，旧的已保存倒计时不再适用，必须丢弃重新计算，
    // 否则会一直按最早那次设置的区间倒计时，改了区间也不会生效
    if (forceRecalculate) {
        localStorage.removeItem('moments_next_post_at');
    }
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    // 红包和问卷卡片不显示操作菜单（转发/收藏/删除图标），和语音字卡保持一致
...
if (!isBatchMode && type === 'normal') {
    window._triggerDelayedReply(true);
        ...
        window._triggerDelayedReply = function(isUserMessage) {
            ...
            // 立即显示 typing indicator（不延迟，确保用户发消息后立刻可见）
        ...
        window.simulateReply = function() {
            function showTypingIndicator() {
                ...
                // 仅在用户已经在底部附近时才滚动
                const _c = DOMElements.chatContainer;
                if (_c && (_c.scrollHeight - _c.scrollTop - _c.clientHeight < 150)) {
...
function showModal(modalElement, focusElement = null) {
            ...
            requestAnimationFrame(() => {
                ...
                if (content) {
                    ...
                    // 每次打开弹窗都滚回顶部，避免停留在上次关闭时的底部位置
                    content.scrollTop = 0;
                    // 同时重置内部所有可滚动容器（信封/.env-body、公告/.dg-body、数据管理/.dm-body 等）
                    content.querySelectorAll('.env-body, .dm-body, .dg-body, .modal-body, .diary-panel, .diary-todo-scroll-area, .diary-habit-scroll-area, .accounting-panel, .accounting-scroll-area, .cs-panels, .mood-options-slider-wrapper, .ann-list-container, [style*="overflow-y:auto"], [style*="overflow-y: auto"], [style*="overflow:auto"], [style*="overflow: auto"]')
