...
autoSendInterval: 5,
        ...
        // 朋友圈自动发布设置
        momentsAutoPostEnabled: false,
        momentsAutoPostMinVal: 30,
        momentsAutoPostMinUnit: 'minutes',
        momentsAutoPostMaxVal: 10,
        momentsAutoPostMaxUnit: 'hours',
        momentsCardCountMin: 3,
        momentsCardCountMax: 5,
        momentsSourceMain: true,
        momentsSourceKaomoji: true,
        momentsSourceEmoji: true,
        momentsSourceSticker: true,
        // 主页绑定会话开关（默认关闭）
...
const loadData = async () => {
    try {
        ...
        setTimeout(() => {
            ...
            // 恢复页面刷新前未完成的回复定时器
            try {
                const pendingAt = sessionStorage.getItem('pending_reply_expected_at');
                if (pendingAt) {
                    const remaining = parseInt(pendingAt) - Date.now();
                    if (remaining > 0 && remaining < 120000) { // 最多等2分钟
                        ...
                        window._pendingReplyTimer = setTimeout(() => {
                            ...
                            sessionStorage.removeItem('pending_reply_expected_at');
            ...
            // settings 已从存储读回，刷新所有依赖 settings 的 UI 开关/滑杆
            // setupEventListeners 执行时 settings 还是默认值，导致刷新后开关被重置
            try { if (typeof updateMomentsAutoPostUI  === 'function') updateMomentsAutoPostUI();  } catch(e) {}
            try { if (typeof updateAutoSendUI         === 'function') updateAutoSendUI();         } catch(e) {}
            try { if (typeof updatePinyinCardUI       === 'function') updatePinyinCardUI();       } catch(e) {}
            try { if (typeof updateMoyuAutoGenerateUI === 'function') updateMoyuAutoGenerateUI(); } catch(e) {}
            try { if (typeof updateMoyuShowDetailUI   === 'function') updateMoyuShowDetailUI();   } catch(e) {}
            try { if (typeof updateEnvelopeSettingsUI === 'function') updateEnvelopeSettingsUI(); } catch(e) {}
        }, 100);
...
let momentsAutoPostTimer = null; // 朋友圈自动发布定时器

function manageMomentsAutoPostTimer() {
    if (momentsAutoPostTimer) {
        clearTimeout(momentsAutoPostTimer);
        momentsAutoPostTimer = null;
    }

    if (!settings.momentsAutoPostEnabled) {
        localStorage.removeItem('moments_next_post_at');
        return;
    }
    ...
    function unitToMs(val, unit) {
        ...
        switch(unit) {
            ...
            default:      return v * 60 * 1000; // minutes
        }
    }

    let minMs, maxMs;
    if (settings.momentsAutoPostMinUnit) {
        // 新版：单位制
        minMs = unitToMs(settings.momentsAutoPostMinVal || 30, settings.momentsAutoPostMinUnit || 'minutes');
        maxMs = unitToMs(settings.momentsAutoPostMaxVal || 10, settings.momentsAutoPostMaxUnit || 'hours');
    } else {
        // 旧版兼容：纯分钟
        const minMinutes = Math.max(1, settings.momentsAutoPostMinMinutes || 30);
        const maxMinutes = Math.max(minMinutes, settings.momentsAutoPostMaxMinutes || 600);
        minMs = minMinutes * 60 * 1000;
        maxMs = maxMinutes * 60 * 1000;
    }
    if (maxMs < minMs) maxMs = minMs;

    // 持久化：恢复已存的下次发送时间（页面刷新后继续倒计时）
    let delay;
    const savedAt = localStorage.getItem('moments_next_post_at');
    if (savedAt) {
        const remaining = parseInt(savedAt) - Date.now();
        if (remaining > 500) {
            delay = remaining; // 继续等待
        } else {
            delay = 0; // 已超时，立即发
        }
    } else {
        delay = minMs + Math.random() * (maxMs - minMs);
        localStorage.setItem('moments_next_post_at', Date.now() + delay);
    }

    momentsAutoPostTimer = setTimeout(() => {
        localStorage.removeItem('moments_next_post_at');
        if (settings.momentsAutoPostEnabled && window.MomentsApp && typeof window.MomentsApp.publishPartnerMoment === 'function') {
            window.MomentsApp.publishPartnerMoment();
        }
        manageMomentsAutoPostTimer(); // 继续安排下一次
    }, Math.max(0, delay));
}
window.manageMomentsAutoPostTimer = manageMomentsAutoPostTimer;
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    let content = (!isXhsCard && !isWebEmbed && !isDQCard && msg.text) ? `<div>${msg.text.replace(/\n/g, '<br>')}</div>` : '';
    if (isVoice) {
        // 微信风格语音消息
    ...
    } else if (isRedPacket || isImageOnly) {
        messageDiv.className = `message message-${msg.sender === 'user' ? 'sent' : 'received'} message-image-bubble-none`;
        messageDiv.innerHTML = messageHTML;
    } else if (isVoice) {
    ...
    if (isVoice) {
        ...
        if (voiceBubble) {
            ...
            let voiceBuffer = null;       // 解码后的 AudioBuffer
            let voiceSource = null;       // 当前 BufferSourceNode
            let voiceCtx = null;          // 播放用 AudioContext（与保活共用或新建）
            let voiceStartTime = 0;       // context 时间轴上的开始时刻
            let voicePauseOffset = 0;     // 暂停时的播放进度（秒）
            let voiceDuration = 0;        // 音频总时长
            const voiceIcon = voiceBubble.querySelector('.voice-icon i');

            // 用 Web Audio API 解码并缓存音频（不触发 iOS 媒体会话）
            const ensureVoiceBuffer = async () => {
                if (voiceBuffer || !voiceUrl) return;
                try {
                    const resp = await fetch(voiceUrl);
                    const ab = await resp.arrayBuffer();
                    // 用一个临时 context 解码，解码完关闭
                    const tmpCtx = new (window.AudioContext || window.webkitAudioContext)();
                    voiceBuffer = await tmpCtx.decodeAudioData(ab);
                    voiceDuration = voiceBuffer.duration;
                    tmpCtx.close();
                    // 更新时长显示
                    const dur = Math.round(voiceDuration);
                    ...
                    voiceBubble._voiceBuffer = voiceBuffer;
                } catch(e) { console.error('语音加载失败:', e); }
            };
            // 提前静默预加载
            ensureVoiceBuffer();
            ...
            const onVoiceEnded = () => {
                ...
                // 播放完后恢复保活音频
                try {
                    if (localStorage.getItem('keepaliveAudioEnabled') === 'true' && window._resumeKeepaliveAudio) {
                        window._resumeKeepaliveAudio();
                    }
                } catch(e) {}
            };
            ...
            voiceBubble.addEventListener('click', function(e) {
                ...
                if (window._currentPlayingVoice && window._currentPlayingVoice !== voiceBubble) {
                    ...
                    if (prev._stopVoice) prev._stopVoice();
                ...
                if (voicePlaying) {
                    // 暂停：记录当前进度
                    voicePauseOffset = voiceCtx ? (voiceCtx.currentTime - voiceStartTime) : 0;
                    stopVoiceSource();
                    voicePlaying = false;
                    voiceBubble._voicePlaying = false;
                    voiceBubble.classList.remove('voice-playing');
                ...
                // 播放：关键点——AudioContext 的创建/resume 必须在点击的同一个事件循环里
                // 同步执行，否则 iOS Safari 会认为这次 resume 已经脱离了用户手势，
                // 导致 context 一直停在 suspended 状态：不报错、没有声音，
                // 而且因为 context 时间不走，音频永远播不完，图标就一直卡在"播放中"。
                if (!voiceCtx || voiceCtx.state === 'closed') {
                    voiceCtx = new (window.AudioContext || window.webkitAudioContext)();
                }
                const resumePromise = voiceCtx.state === 'suspended' ? voiceCtx.resume() : Promise.resolve();
                ...
                (async () => {
                    await resumePromise;
                    // 确保 buffer 已加载（一般渲染时已经预加载好了，这里只是兜底）
                    await ensureVoiceBuffer();

                    if (!voiceBuffer || voiceCtx.state !== 'running') {
                        // 加载失败或者 context 仍未恢复，恢复按钮状态并提示，而不是卡住不动
                        voicePlaying = false;
                        voiceBubble._voicePlaying = false;
                        voiceBubble.classList.remove('voice-playing');
                        if (voiceIcon) voiceIcon.className = 'fas fa-play';
                        if (window._currentPlayingVoice === voiceBubble) window._currentPlayingVoice = null;
                        console.error('语音播放失败：音频未加载或 AudioContext 未能启动');
                        if (typeof showNotification === 'function') showNotification('语音播放失败，请再点一次试试', 'error', 2000);
                        return;
                    }

                    stopVoiceSource();
                    voiceSource = voiceCtx.createBufferSource();
                    voiceSource.buffer = voiceBuffer;
                    voiceSource.connect(voiceCtx.destination);
                    voiceSource.onended = onVoiceEnded;
                    voiceStartTime = voiceCtx.currentTime - voicePauseOffset;
                    voiceSource.start(0, voicePauseOffset);
                    voiceBubble._stopVoice = () => { stopVoiceSource(); voicePlaying = false; voicePauseOffset = 0; };

                    // 兜底：万一 onended 因为某些浏览器怪异行为没有触发，
                    // 到了音频时长后强制把状态复位，避免图标永远卡在"播放中"
                    const remainingMs = Math.max(0, (voiceDuration - voicePauseOffset) * 1000) + 300;
                    const currentSourceRef = voiceSource;
                    setTimeout(() => {
                        if (voiceSource === currentSourceRef && voicePlaying) {
                            onVoiceEnded();
                        }
                    }, remainingMs);
            ...
            // "转文字"按钮：单独一行，显示在气泡正下方
            if (voiceText) {
                const transBtn = document.createElement('div');
                transBtn.className = 'voice-trans-btn';
                transBtn.textContent = '转文字';
                // 插入到 messageDiv 之后（气泡下方）
                messageDiv.after(transBtn);
                transBtn.addEventListener('click', function(e) {
                    ...
                    if (existing) {
                        ...
                        transBtn.after(transcriptDiv);
                        transBtn.textContent = '收起';
...
// 供外部模块（xhs、web-embed 等）调用，触发「对方已读」逻辑
window.scheduleReadReceipt = function() {
    if (!settings || !settings.readReceiptsEnabled) return;
...
if (!isBatchMode && type === 'normal') {
    ...
    function _hideTypingNow() {
        try {
            ...
            if (_tiW) {
                const _tiInner = _tiW.querySelector('.typing-indicator');
                if (_tiInner) { _tiInner.classList.add('hiding'); setTimeout(() => { _tiW.style.display='none'; _tiInner.classList.remove('hiding'); }, 240); }
                else _tiW.style.display = 'none';
            }
        } catch(e) {}
    }

    // 「发送失败」通知：优先发手机系统推送，降级才用网页内弹窗
    function _showSendFailedNotification(reason) {
        try {
            const partnerName = settings.partnerName || '对方';
            const bodyText = reason === 'ignore'
                ? partnerName + ' 没有回复这条消息'
                : partnerName + ' 的回复超时未送达';
            ...
            if ('Notification' in window && Notification.permission === 'granted') {
                try {
                    const notifOpts = {
                        body: bodyText,
                        icon: partnerAvatar || undefined,
                        tag: 'send-failed',          // 同类通知只保留最新一条
                        renotify: true,
                        requireInteraction: false,
                        silent: false
                    };
                    const n = new Notification(partnerName + ' 的消息未发出', notifOpts);
                    n.onclick = () => { window.focus(); n.close(); };
                } catch(e) {
                    console.warn('[SendFailed] 系统通知失败，降级网页弹窗:', e);
                    _showSendFailedInPage(partnerName, bodyText, partnerAvatar);
                }
            } else {
                // ② 降级：网页内弹窗（没有通知权限 / 不支持时）
                _showSendFailedInPage(partnerName, bodyText, partnerAvatar);
            }
        } catch(e) {}
    }
    ...
    // 网页内降级弹窗
    function _showSendFailedInPage(partnerName, bodyText, partnerAvatar) {
        try {
            const avatarHtml = partnerAvatar
                ? `<img src="${partnerAvatar}" style="width:36px;height:36px;border-radius:50%;object-fit:cover;">`
                : `<div style="width:36px;height:36px;border-radius:50%;background:rgba(var(--accent-color-rgb),0.15);display:flex;align-items:center;justify-content:center;"><i class="fas fa-user" style="color:var(--accent-color);font-size:16px;"></i></div>`;
            const existing = document.getElementById('_send_failed_notification');
            if (existing) existing.remove();
            const el = document.createElement('div');
            el.id = '_send_failed_notification';
            el.innerHTML = `
                <div style="position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:10000;background:var(--secondary-bg);border:1px solid var(--border-color);border-radius:16px;padding:16px 20px;box-shadow:0 8px 32px rgba(0,0,0,0.2);max-width:360px;width:88%;font-family:var(--font-family);">
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                        <div style="position:relative;flex-shrink:0;">
                            ${avatarHtml}
                            <div style="position:absolute;bottom:-2px;right:-2px;width:16px;height:16px;border-radius:50%;background:#ff4d4f;display:flex;align-items:center;justify-content:center;border:2px solid var(--secondary-bg);">
                                <i class="fas fa-times" style="color:white;font-size:8px;"></i>
                            </div>
                        </div>
                        <div style="flex:1;min-width:0;">
                            <div style="font-size:14px;font-weight:600;color:var(--text-primary);">${partnerName} 的消息未发出</div>
                            <div style="font-size:12px;color:var(--text-secondary);margin-top:2px;">${bodyText}</div>
                        </div>
                        <button onclick="document.getElementById('_send_failed_notification').remove()" style="background:none;border:none;color:var(--text-secondary);cursor:pointer;padding:4px;font-size:16px;flex-shrink:0;">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    <div style="font-size:12px;color:var(--text-secondary);padding:8px 10px;background:var(--primary-bg);border-radius:8px;line-height:1.5;">
                        <i class="fas fa-info-circle" style="color:#ff9800;margin-right:6px;"></i>对方正在输入的气泡已消失，但这条回复没有成功发送给你
                    </div>
                </div>
            `;
            document.body.appendChild(el);
            setTimeout(() => {
                const n = document.getElementById('_send_failed_notification');
                if (n) n.remove();
            }, 3000);
        } catch(e) {}
    }

    function _queueSendFailedNotification(reason) {
        try { sessionStorage.setItem('_send_failed_pending', reason); } catch(e) {}
        _showSendFailedNotification(reason);
    }
    // 暴露到全局，供 app.js 回到前台时调用
    window._showSendFailedNotificationGlobal = _showSendFailedNotification;
    ...
    if (!shouldIgnore) {
        if (settings.typingIndicatorEnabled) {
            ...
            // 安全兜底：输入中气泡最多显示 randomDelay + 3s，防止卡死
            if (window._typingIndicatorAutoHideTimer) clearTimeout(window._typingIndicatorAutoHideTimer);
            window._typingIndicatorAutoHideTimer = setTimeout(() => {
                _hideTypingNow();
                // 兜底超时 = 消息没发出来，弹「未发出」通知
                _queueSendFailedNotification('timeout');
            }, randomDelay + 3000);
        }
        // 持久化：记录预期回复时间，页面刷新后可恢复
        const expectedAt = Date.now() + randomDelay;
        try { sessionStorage.setItem('pending_reply_expected_at', expectedAt); } catch(e) {}
        window._pendingReplyTimer = setTimeout(() => {
            window._pendingReplyTimer = null;
            try { sessionStorage.removeItem('pending_reply_expected_at'); } catch(e) {}
            simulateReply();
        }, randomDelay);
    } else {
        // 已读不回：确保隐藏遗留的输入中气泡，并弹「未发出」通知
        _hideTypingNow();
        _queueSendFailedNotification('ignore');
        try { sessionStorage.removeItem('pending_reply_expected_at'); } catch(e) {}
    }
}
};
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            for (let i = 0; i < replyCount; i++) {
                if (i === 0) {
                    // 第一条消息：完整使用用户设置的「回复速度」区间
                    const delayRange = settings.replyDelayMax - settings.replyDelayMin;
                    delay = settings.replyDelayMin + Math.random() * delayRange;
                } else {
                    // 后续连发的消息：只加一个较短的自然间隔，不再叠加一整段「回复速度」，
                    // 避免总等待时间变成设置值的两三倍
