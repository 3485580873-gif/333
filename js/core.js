...
autoSendInterval: 5,
        ...
        momentsAutoPostMinMinutes: 30,
        momentsAutoPostMaxMinutes: 600,
...
function manageMomentsAutoPostTimer() {
    ...
    if (!settings.momentsAutoPostEnabled) return;

    const minMinutes = Math.max(30, settings.momentsAutoPostMinMinutes || 30);
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    let content = (!isXhsCard && !isWebEmbed && msg.text) ? `<div>${msg.text.replace(/\n/g, '<br>')}</div>` : '';
    if (isVoice) {
        // 微信风格语音消息
