...
if (!isBatchMode && type === 'normal') {
    window._triggerDelayedReply(true);
        ...
        // 通用：触发对方延迟回复机制（从参考版本移植）
        //   isUserMessage=true 表示真实用户消息；false 表示陪伴/自动触发
        window._triggerDelayedReply = function(isUserMessage) {
            ...
            try {
                ...
                var _tiWc = document.getElementById('typing-indicator-wrapper');
                if (_tiWc) _tiWc.style.display = 'none';
            } catch(e) {}
            ...
            // 「正在输入」气泡在消息到来前约 1.2 秒才出现，避免长时间悬挂
            const typingLeadTime = Math.min(1200, Math.max(0, randomDelay - 800));
            const typingShowDelay = randomDelay - typingLeadTime;
            const _replyToken = (window._replyToken = (window._replyToken || 0) + 1);
            if (settings.typingIndicatorEnabled && typingLeadTime > 0) {
                setTimeout(() => {
                    if (window._replyToken !== _replyToken) return;
