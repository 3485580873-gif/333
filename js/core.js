...
if (!isBatchMode && type === 'normal') {
    ...
    // 「发送失败」通知已关闭：超时时自动重发，不弹任何通知
    function _showSendFailedNotification(reason) {
        // 已禁用：不弹网页内弹窗，也不发手机系统推送
    }
    ...
    function _queueSendFailedNotification(reason) {
        try { sessionStorage.removeItem('_send_failed_pending'); } catch(e) {}
        // timeout 时自动触发继续发信息，ignore（已读不回）时静默忽略
        if (reason === 'timeout') {
            try {
                if (typeof simulateReply === 'function') simulateReply();
            } catch(e) { console.warn('[AutoResume] 自动继续发信息失败:', e); }
        }
    }
    // 暴露到全局，供 app.js 回到前台时调用（已禁用弹窗，仅保留接口兼容）
    window._showSendFailedNotificationGlobal = function() {};

    if (!shouldIgnore) {
        // 确保上一轮遗留的气泡已清除，不提前显示"正在输入"——
        // 气泡由 simulateReply 在即将发出消息前才展示，避免气泡长时间悬挂
        _hideTypingNow();
        // 安全兜底定时器：若 _pendingReplyTimer 因故未能触发，到期后强制调用 simulateReply
        ...
        window._typingIndicatorAutoHideTimer = setTimeout(() => {
            ...
            if (window._pendingReplyTimer) {
                ...
                try {
                    if (typeof simulateReply === 'function') simulateReply();
                } catch(e) { console.warn('[AutoResume] 兜底超时自动继续发信息失败:', e); }
            }
        }, randomDelay + 3000);
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            showTypingIndicator();
            // 预先算好所有消息延迟，auto-hide 定时器覆盖最后一条，气泡全程可见
            const _msgDelays = [];
            { let _acc = 800 + Math.random() * 1200; _msgDelays.push(_acc);
              for (let _j = 1; _j < replyCount; _j++) { _acc += 1200 + Math.random() * 1800; _msgDelays.push(_acc); } }
            const _lastMsgDelay = _msgDelays[_msgDelays.length - 1];
            if (window._typingIndicatorAutoHideTimer) clearTimeout(window._typingIndicatorAutoHideTimer);
            window._typingIndicatorAutoHideTimer = setTimeout(function() {
                window._typingIndicatorAutoHideTimer = null;
                var _tiW = document.getElementById('typing-indicator-wrapper');
                if (_tiW) { var _ti = _tiW.querySelector('.typing-indicator'); if (_ti) { _ti.classList.add('hiding'); setTimeout(function(){ _tiW.style.display='none'; if (_ti) _ti.classList.remove('hiding'); }, 240); } else { _tiW.style.display='none'; } }
            }, _lastMsgDelay + 600);
            let delay = 0;
