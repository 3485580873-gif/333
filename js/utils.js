    ...
    return { result, removedCount };
        ...
        const playSound = (type) => {
            ...
            try {
                ...
                osc1.stop(end);
                osc2.stop(end);
                // 振荡器停止后关闭 AudioContext，避免持有活跃音频上下文影响后台保活
                const closeDelay = Math.ceil(cfg.dur * 1000) + 100;
                setTimeout(() => {
                    try {
                        if (audioContext.state !== 'closed') audioContext.close();
                    } catch(e) {}
                    if (_currentAudioContext === audioContext) _currentAudioContext = null;
                }, closeDelay);
    ...
    try {
        ...
        const categories = [
            ...
            },
            {
                id: 'voices',
                label: '语音字卡',
                indexedDBNeedles: ['customVoices', 'customVoiceGroups'],
                localStorageNeedles: []
            },
            {
                id: 'stickers',
            ...
            {
                id: 'home',
                label: '主页外观（背景 / 卡片 / 图标 / 火花 / 头像）',
                indexedDBNeedles: ['home_page_bg_custom', 'home_card_bg_custom', 'home_avatar_me', 'home_avatar_partner', 'home_app_icons', 'callBgImageData'],
                localStorageNeedles: ['home_page_bg', 'home_card_bg', 'home_icon_color', 'home_icon_color_name', 'home_hero_subtitle', 'home_theme', 'home_theme_custom', 'home_app_icons', 'home_app_order', 'home_avatar_me', 'home_avatar_partner', 'home_card_bg_custom', 'home_page_bg_custom', 'home_nav_colors', 'home_card_style', 'home_layout', 'profile_me', 'profile_partner', 'chat_streak_data', 'home_session_bind', 'home_avatar_sync', 'home_bg_sync', 'home_profile_']
            }
        ];
