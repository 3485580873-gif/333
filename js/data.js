            ...
            if (window.localforage) {
                localforage.keys().then(function (keys) {
                    var promises = keys.map(function (k) {
                        ...
                        // 陪伴背景/音频数据也是大文件，同样只估算不读内容
                        if (k.includes('companionBg_') || k.includes('companionNoise_') || k.includes('companionVoice_')) {
                            return localforage.getItem(k).then(function(raw) {
                                var bytes = typeof raw === 'string' ? raw.length * 2 : 0;
                                return { k: k, b: bytes };
                            }).catch(function() { return { k: k, b: 0 }; });
                        ...
                        return localforage.getItem(k).then(function (raw) {
                            ...
                            // 超过4MB的数据只取长度，不做完整clone
                            if (str && str.length > 4 * 1024 * 1024) return { k: k, b: (k.length + str.length) * 2 };
                ...
                var promises = keys.map(function(k) {
                    ...
                    if (k.startsWith('favAudio_')) {
                        return localforage.getItem(k).then(function(v) {
                            var bytes = typeof v === 'string' ? v.length * 2 : 0;
                            return bytes;
