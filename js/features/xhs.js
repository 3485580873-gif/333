...
async function parseXhsLink(url) {
    ...
    // 规范化 API 响应（兼容各种字段结构，文字帖/图文帖/视频帖都支持）
    function normalize(data) {
        // 放宽成功判断：只要返回了数据就认为成功
        const ok = data.code === 200 || data.code === '200' || data.code === 0 ||
                   data.success === true || data.status === 200 || data.status === 'ok' ||
                   (data.data && typeof data.data === 'object') ||
                   data.title || data.desc || data.content || data.text;
        if (!ok) throw new Error(data.msg || data.message || data.error || '解析失败');
        // 支持多层数据结构
        const d = data.data || data.result || data.note || data;
        // 提取图片列表（兼容多种字段名和嵌套结构）
        const rawMedia = d.images || d.imgs || d.picList || d.image_list ||
                         d.imageList || d.pic_list || d.media || d.photos || [];
        const media = Array.isArray(rawMedia)
            ? rawMedia.map(img => (typeof img === 'string' ? img : (img.url || img.src || img.urlDefault || img.infoList?.[0]?.url || '')))
                       .filter(Boolean)
            : [];
        return {
            title:    d.title || d.note_title || d.name || '小红书笔记',
            desc:     d.desc  || d.content   || d.text || d.note_text ||
                      d.body  || d.detail    || d.note_content || d.introduce || '',
            author: {
                name:   d.author?.name     || d.author?.nickname || d.author?.nickName ||
                        d.nickname         || d.nickName         || d.user?.nickname   ||
                        d.blogger?.name    || (typeof d.author === 'string' ? d.author : '') || '小红书用户',
                avatar: d.author?.avatar   || d.author?.headimg || d.author?.avatarUrl ||
                        d.headimg          || d.avatar          || d.user?.avatar      ||
                        d.blogger?.avatar  || ''
            },
            media,
            video:    d.video || d.videoUrl || d.video_url || d.videoLink || null,
            likes:    d.likes    || d.likeCount    || d.like_count    || d.liked    || 0,
            comments: d.comments || d.commentCount || d.comment_count || d.commented|| 0,
            collects: d.collects || d.collectCount || d.collect_count || d.saved    || 0,
            time:     d.time     || d.createTime   || d.create_time   || d.pubTime  || '',
            raw: data
        };
    }

    // ── 并行竞速：所有 API × 所有方式同时发出，谁先成功就用谁 ──
    const controllers = [];
    ...
    for (const api of apiEndpoints) {
        ...
        // 直连（超时 5s）
        allRequests.push(
            fetch(targetUrl, { signal: makeSignal(5000) })
        ...
        for (const proxy of corsProxies) {
            const { url: pUrl, extract } = proxy(targetUrl);
            allRequests.push(
                fetch(pUrl, { signal: makeSignal(8000) })
                    .then(r => { if (!r.ok) throw new Error(`代理 HTTP ${r.status}`); return r.json(); })
                    .then(d => normalize(extract(d)))
            );
        }
    }

    try {
        // 第一个成功的结果立刻返回，取消其余请求节省流量
        const result = await Promise.any(allRequests);
