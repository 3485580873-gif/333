...
(function () {
    ...
    function openCompanionPage(opts) {
        ...
        // 隐藏body下所有其他直接子元素，防止任何层叠穿透
        Array.from(document.body.children).forEach(el => {
            if (el !== page && !el.dataset._companionSkip) {
                el.dataset._companionHidden = el.style.display || '';
    ...
    function closeCompanionPage(opts) {
        ...
        // 恢复所有被隐藏的body子元素
        Array.from(document.body.children).forEach(el => {
            if ('_companionHidden' in el.dataset) {
                el.style.display = el.dataset._companionHidden || '';
                delete el.dataset._companionHidden;
            }
        });
