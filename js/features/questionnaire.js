...
function _addDQChatMessage(dq, sender) {
    ...
    const cardHtml = `<div style="display:flex;flex-direction:column;gap:0;min-width:180px;max-width:220px;background:transparent;">
        <div style="background:var(--accent-color,#07c160);color:#fff;border-radius:10px 10px 0 0;padding:10px 14px;">
            <div style="font-size:13px;font-weight:700;margin-bottom:2px;">📋 梦向问卷</div>
            <div style="font-size:11px;opacity:0.85;">${escapeHtml(title)}</div>
        </div>
        <div style="background:#fff;border:1px solid #e5e5e5;border-top:none;border-radius:0 0 10px 10px;padding:8px 14px;font-size:12px;color:#666;">
            ${qCount} 题 · ${typeLabel}
        </div>
    </div>`;
    const msg = {
...
function _addDQAnswerChatMessage(dq) {
    ...
    const answersHtml = (dq.answer.answers || []).map((a, i) => {
        return `<div style="margin-bottom:6px;">
            ...
            <div style="font-size:13px;color:#333;font-weight:500;">${escapeHtml(a.answer)}</div>
        </div>`;
    }).join('');
    const cardHtml = `<div style="display:flex;flex-direction:column;gap:0;min-width:180px;max-width:240px;background:transparent;">
        <div style="background:linear-gradient(135deg,#ff7eb3,#ff758c);color:#fff;border-radius:10px 10px 0 0;padding:10px 14px;">
            <div style="font-size:13px;font-weight:700;margin-bottom:2px;">✅ 已填写问卷</div>
            <div style="font-size:11px;opacity:0.9;">${escapeHtml(title)}</div>
        </div>
        <div style="background:#fff;border:1px solid #e5e5e5;border-top:none;border-radius:0 0 10px 10px;padding:10px 14px;">
            ${answersHtml}
        </div>
    </div>`;
    const msg = {
        ...
        ${dq.answer.answers.map((a, i) => {
            ...
            <div class="dq-qa-item">
                ...
                ${dq.type === 'choice' && origOptions.length > 0
                    ? `<div style="margin-top:8px;display:flex;flex-direction:column;gap:6px;">
                        ${origOptions.map((opt, oi) => {
                            ...
                            return `<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;font-size:13px;
                                ...
                                <span style="width:20px;height:20px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:11px;flex-shrink:0;
                    ...
                    : `<div class="dq-qa-answer fill-answer">${escapeHtml(a.answer)}</div>`
                }
            </div>`;
        }).join('')}
    `;
