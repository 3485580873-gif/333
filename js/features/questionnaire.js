    ...
    const answers = dq.questions.map(q => {
        if (dq.type === 'choice') {
            ...
            if (q.selectionMode === 'multi') {
                ...
                return { question: q.question, answer: shuffled.slice(0, pickCount), selectionMode: 'multi' };
            }
            const randomIndex = Math.floor(Math.random() * options.length);
            return { question: q.question, answer: options[randomIndex], selectionMode: 'single' };
        } else {
...
function viewDQAnswer(id) {
    ...
    answerContent.innerHTML = `
        ...
        ${dq.answer.answers.map((a, i) => {
            ...
            const selectedArr = Array.isArray(a.answer) ? a.answer : (a.answer ? [a.answer] : []);
            return `
...
function openDQChatDetail(dqId) {
    ...
    body.innerHTML = (dq.questions || []).map((q, i) => {
        ...
        const selectedArr = Array.isArray(rawSelected) ? rawSelected : (rawSelected ? [rawSelected] : []);
