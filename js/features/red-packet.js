    ...
    window.showRedPacketReceiveModal = function (recordId) {
        ...
        if (openBtn && isPending) {
            openBtn.onclick = function() {
                ...
                // 原地更新对方那张卡片状态，避免全量 renderMessages 导致闪屏
                var existingCard = document.querySelector('.red-packet-card[data-rp-id="' + record.id + '"]');
                if (existingCard && typeof window.renderRedPacketMessage === 'function') {
                    var isSentBubble = !!existingCard.closest('.message-sent');
                    var tmp = document.createElement('div');
                    tmp.innerHTML = window.renderRedPacketMessage({
                        sender: isSentBubble ? 'user' : 'partner',
                        id: record.id,
                        redPacket: record,
                        text: record.message,
                ...
                // 添加我方已领取回执卡片（addMessage 自带平滑滚底，不会闪）
                if (typeof addMessage === 'function') {
                    addMessage({
                        id: 'rp_recv_card_' + Date.now(),
                        sender: 'user',
