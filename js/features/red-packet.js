...
(function () {
    ...
    window.showRedPacketReceiveModal = function (recordId) {
        ...
        var btnBg = isOpened
            ...
            : 'background:#ffd700;color:#3a9f8a;box-shadow:0 2px 10px rgba(255,215,0,0.5);cursor:pointer;';
        ...
        var html =
            '<div id="rp-receive-panel" style="text-align:center;position:relative;overflow:hidden;border-radius:16px;width:260px;min-height:380px;' + panelBg + 'display:flex;flex-direction:column;">' +
                ...
                '</div>' +
                // 底部按钮区域
                '<div style="padding:30px 20px 40px;display:flex;flex-direction:column;align-items:center;justify-content:center;' + (isOpened ? 'background:#ccc;' : 'background:#e88fa7;') + '">' +
            ...
            openBtn.onclick = function () {
                ...
                panel.innerHTML =
                    ...
                    '<div style="padding:30px 16px 20px;display:flex;flex-direction:column;align-items:center;flex:1;justify-content:center;">' +
                        ...
                        '<div style="font-size:24px;font-weight:700;color:#3a9f8a;margin-top:8px;">&yen;' + fmt(record.amount) + '</div>' +
                    '</div>' +
    ...
    window.renderRedPacketMessage = function (msg) {
        ...
        var statusHtml = '';
        if (status === 'pending') {
            statusHtml = '<span style="display:flex;align-items:center;gap:4px;font-weight:500;color:#3a9f8a;"><i class="fas fa-clock" style="font-size:10px;"></i> ' + (isSentByMe ? '对方待领取' : '待领取') + '</span>';
        ...
        var bodyBg = isOpened
            ? 'background:linear-gradient(180deg,#e0d8d8 0%,#ccc 100%);'
            : 'background:linear-gradient(180deg,#5db8a8 0%,#3a8f7d 100%);';

        var svgStroke = isOpened ? 'stroke="#999"' : 'stroke="#fff"';
        var svgCircleFill = isOpened ? 'fill="#999"' : 'fill="#fff"';
        var rpSvgCustom = '<svg width="36" height="44" viewBox="0 0 20 28" fill="none" ' + svgStroke + ' stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="5" width="18" height="14" rx="2.5"/><path d="M1 9h18M5 13h4" stroke-linecap="round"/><circle cx="15" cy="13" r="1.8" ' + svgCircleFill + ' stroke="none"/></svg>';
            ...
            '<div class="red-packet-card' + (isOpened ? ' opened' : '') + '" data-rp-id="' + recordId + '" style="width:260px;border-radius:6px;overflow:hidden;cursor:pointer;transition:transform 0.15s;position:relative;">' +
                ...
                '<div class="rp-body" style="' + bodyBg + 'padding:12px 14px 14px;color:#fff;position:relative;display:flex;align-items:center;gap:12px;">' +
                    ...
                    '<div class="rp-content" style="flex:1;min-width:0;">' +
                        '<div class="rp-title" style="font-size:13px;font-weight:600;margin-bottom:2px;">转账</div>' +
