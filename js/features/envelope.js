...
let editingEnvId = null;let envelopeData = { outbox: [], inbox: [], spacetime: [] };
let currentEnvTab = 'outbox';
...
window.viewEnvLetter = function(section, id) {
    ...
    // 将 envelope-view-modal 移入 envelope-modal 内部覆盖显示，
    // 避免调用全局 showModal/homeShowModal 导致界面错位
    const viewModal = document.getElementById('envelope-view-modal');
    const envelopeModal = document.getElementById('envelope-modal');
    const envelopeContent = envelopeModal ? envelopeModal.querySelector('.modal-content') : null;

    if (envelopeContent && viewModal) {
        // 若 viewModal 不在 envelopeContent 内，移入
        if (viewModal.parentElement !== envelopeContent) {
            envelopeContent.appendChild(viewModal);
        }
        // 以绝对定位覆盖在 envelope-modal 内容区上方
        viewModal.style.cssText = 'display:flex;position:absolute;top:0;left:0;width:100%;height:100%;z-index:10;align-items:center;justify-content:center;background:rgba(0,0,0,0.35);border-radius:inherit;';
        // 触发内容区入场动画
        const content = viewModal.querySelector('.modal-content');
        if (content) {
        ...
        // 确保外层 envelopeContent 有相对定位
        if (getComputedStyle(envelopeContent).position === 'static') {
            envelopeContent.style.position = 'relative';
        }
    } else {
        // 降级：直接显示
        viewModal.style.cssText = 'display:flex;position:fixed;top:0;left:0;width:100%;height:100%;z-index:100001;align-items:center;justify-content:center;background:rgba(0,0,0,0.6);';
    }
};
