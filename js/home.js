...
(function() {
    ...
    const defaultAppOrder = ['chat', 'mailbox', 'moyu', 'diary', 'fortune', 'mood', 'calendar', 'decide', 'stats', 'ta-phone'];
    ...
    async function initWidgets() {
        // 在一起天数：从纪念日数据或 chatSettings 读取（localStorage + localforage 都检查）
        try {
            ...
            if (daysEl) {
                ...
                // 0. 用户手动设置的日期（优先级最高，完全独立）
                const manualDate = localStorage.getItem('home_love_date_manual');
                if (manualDate) {
                    days = calcDays(manualDate + 'T00:00:00');
                }

                // 1. diaryAnniversaries
                const anniversaryData = localStorage.getItem('diaryAnniversaries') || localStorage.getItem('anniversaries');
                if (anniversaryData) {
                    const arr = JSON.parse(anniversaryData);
                    const main = arr.find(a => a.isMain || a.main || a.type === 'love') || arr[0];
                    if (main && main.date) days = calcDays(main.date);
                }

                // 2. chatSettings（localStorage）
                if (days === null) {
                    const cs = localStorage.getItem('chatSettings');
                    if (cs) {
                        const parsed = JSON.parse(cs);
                        days = calcDays(parsed.toggleDate || parsed.startDate || parsed.loveDate);
                    }
                }

                // 3. chatSettings（localforage，因为 core.js 存在这里）
                if (days === null && typeof localforage !== 'undefined') {
                    try {
                        // 尝试所有可能的 session key 格式
                        const keys = await localforage.keys();
                        const csKey = keys.find(k => k.includes('chatSettings'));
                        if (csKey) {
                            const lf = await localforage.getItem(csKey);
                            if (lf && typeof lf === 'object') {
                                days = calcDays(lf.toggleDate || lf.startDate || lf.loveDate);
                            }
                        }
                    } catch(e) {}
                }

                // 4. 全局 settings 对象（core.js 加载后可能直接可访问）
                if (days === null && typeof settings !== 'undefined') {
                    days = calcDays(settings.toggleDate || settings.startDate || settings.loveDate);
                }
    ...
    window.handleBgUpload = function(event) {
        ...
        compressP.then(async function(url) {
            ...
            // 同步到 hero 卡片背景
            const heroBg = document.getElementById('hero-bg-inner');
            if (heroBg) heroBg.style.background = bgValue;
            // 同步到天数小组件内容区
            const dwBg = document.getElementById('days-widget-bg');
            if (dwBg) {
                dwBg.style.backgroundImage = `url(${url})`;
                dwBg.style.backgroundSize = 'cover';
                dwBg.style.backgroundPosition = 'center';
            }
            // 同步到所有 home-widget（保留兼容性）
            document.querySelectorAll('.home-widget').forEach(function(w) {
    ...
    function renderIconGrid() {
        ...
        grid.innerHTML = '';
        const REMOVED_APPS = ['accounting', 'map', 'shop', 'gift-cabinet', 'pet'];
        const nameMap = { chat:'聊天', mailbox:'信封', moyu:'摸鱼', diary:'朝夕心记', fortune:'运势', mood:'心晴', calendar:'日历', decide:'抉择', stats:'统计', accounting:'记账', shop:'商城', 'gift-cabinet':'礼物柜', pet:'萌宠屋', 'ta-phone':'TA的手机', map:'Zmilk地图' };

        Object.keys(defaultAppIcons).filter(app => !REMOVED_APPS.includes(app)).forEach(app => {
    ...
    window.showMomentsPage = function() {
        ...
        // 隐藏聊天相关的 fixed 浮动按钮（防止遮挡朋友圈评论按钮）
        var backBtn = document.getElementById('back-to-latest-btn');
        ...
        if (momentsContainer) {
            ...
            // 每次打开都从顶部开始，不保留上次滚动位置
            momentsContainer.scrollTop = 0;
        }
    ...
    async function loadSavedSettings() {
        ...
        if (savedBg === 'custom') {
            ...
            if (customUrl) {
                ...
                // 同步到天数小组件
                const dwBg2 = document.getElementById('days-widget-bg');
                if (dwBg2) {
                    dwBg2.style.backgroundImage = `url(${customUrl})`;
                    dwBg2.style.backgroundSize = 'cover';
                    dwBg2.style.backgroundPosition = 'center';
                }
                // 同步到所有 home-widget
                document.querySelectorAll('.home-widget').forEach(function(w) {
                    w.style.backgroundImage = `url(${customUrl})`;
                    w.style.backgroundSize = 'cover';
                    w.style.backgroundPosition = 'center';
                });
                if (heroBg) heroBg.style.background = `url(${customUrl}) center/cover no-repeat`;
        ...
        if (savedIcons) {
            ...
            // localStorage 没有（可能因备份恢复时超限只写了 localforage），从 localforage 异步补读
            homeGetLargeItem('home_app_icons').then(function(lfVal) {
                if (!lfVal) return;
                try {
                    const lfIcons = typeof lfVal === 'string' ? JSON.parse(lfVal) : lfVal;
                    customAppIcons = lfIcons;
                    // 同步回 localStorage，下次就能直接读到
                    try { homeSetItem('home_app_icons', JSON.stringify(lfIcons)); } catch(e) {}
                    Object.keys(lfIcons).forEach(function(app) {
                        const iconEl = document.querySelector(`.app-icon[data-app="${app}"]`);
                        if (iconEl && lfIcons[app]) {
                            iconEl.innerHTML = `<img src="${lfIcons[app]}" style="width:100%;height:100%;object-fit:cover;border-radius:12px;">`;
                        }
                    });
                } catch(e) {}
            }).catch(function() {});
        ...
        if (savedOrder) {
            try {
                appOrder = JSON.parse(savedOrder).filter(id => !REMOVED_APPS.includes(id));
                reorderAppItems();
            } catch(e) {}
        } else {
            // 首次加载：按默认顺序重新分页（每页8个）并保存
            appOrder = appOrder.filter(id => !REMOVED_APPS.includes(id));
            reorderAppItems();
            saveAppOrder();
        }
    ...
    // ========== 手动设置在一起天数 ==========
    window.showDaysSetModal = function() {
        var modal = document.getElementById('days-set-modal');
        if (!modal) return;
        var inp = document.getElementById('days-date-input');
        var saved = localStorage.getItem('home_love_date_manual');
        if (inp) {
            inp.value = saved || new Date().toISOString().slice(0, 10);
            inp.max = new Date().toISOString().slice(0, 10);
        }
        modal.style.display = 'flex';
    };

    window.saveLoveDate = function() {
        var inp = document.getElementById('days-date-input');
        if (!inp || !inp.value) return;
        localStorage.setItem('home_love_date_manual', inp.value);
        // 立即更新天数显示
        var diff = Math.floor((Date.now() - new Date(inp.value + 'T00:00:00').getTime()) / 86400000);
        var el = document.getElementById('home-days-count');
        if (el) el.textContent = diff >= 0 ? diff : '—';
        document.getElementById('days-set-modal').style.display = 'none';
        if (typeof showNotification === 'function') showNotification('在一起日期已保存 💗', 'success', 1800);
    };
