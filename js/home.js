...
(function() {
    ...
    function initWidgets() {
        ...
        try {
            ...
            if (daysEl) {
                const anniversaryData = localStorage.getItem('diaryAnniversaries') ||
                    localStorage.getItem('anniversaries') || null;
                let days = '—';
                if (anniversaryData) {
                    const arr = JSON.parse(anniversaryData);
                    const main = arr.find(a => a.isMain || a.main || a.type === 'love' || arr[0]);
                    if (main && main.date) {
                        const diff = Math.floor((Date.now() - new Date(main.date).getTime()) / 86400000);
                        if (diff >= 0) days = diff;
                    }
                }
                // 尝试从 chatSettings 读取
                if (days === '—') {
                    ...
                    if (cs) {
                        ...
                        const d = parsed.toggleDate || parsed.startDate || parsed.loveDate;
                        if (d) {
        ...
        // 来自TA的悄悄话：读最新信封消息
        try {
            const msgEl = document.getElementById('home-msg-preview');
            if (msgEl) {
                let preview = '还没有悄悄话~';
                const envRaw = localStorage.getItem('envelopeData');
                if (envRaw) {
                    const env = JSON.parse(envRaw);
                    const inbox = (env.inbox || []).filter(e => e.content || e.text || e.message);
                    if (inbox.length > 0) {
                        const latest = inbox[inbox.length - 1];
                        const text = latest.content || latest.text || latest.message || '';
