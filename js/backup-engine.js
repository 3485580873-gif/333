...
(function (global) {
    ...
    function processLocalStorageValueForImport(str, store) {
        ...
        try {
            ...
            // 如果整个值本来就是顶层的一个 __mRef 引用（比如头像、朋友圈封面、主页背景这种
            // "值本身就是一张图"的键），inlineMediaTree 会直接返回还原后的原始 base64 字符串。
            // 这种情况绝对不能再套一层 JSON.stringify，否则会把字符串错误地包上一对引号，
            // 变成非法的 data URL，导致头像/背景/封面图恢复后全部失效、无法显示。
            // 只有当结果仍然是对象/数组（比如 home_app_icons 这种"多张图打包在一个对象里"的键）
            // 时才需要重新 JSON.stringify 成字符串写回 localStorage。
    ...
    /**
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            // 跳过陪伴系统的背景/音频数据（base64大文件，读入内存会崩溃）
            if (key.includes('companionBg_') || key.includes('companionNoise_') || key.includes('companionVoice_')) continue;
            // favAudio_ 是音频Base64，仅读取时才会占用内存，备份时跳过避免崩溃
            if (key.startsWith('favAudio_')) continue;
            try {
                var rawVal = await localforage.getItem(key);
                if (rawVal === null || rawVal === undefined) continue;
                // 超过8MB的值跳过，防止内存爆炸
                var valStr = typeof rawVal === 'string' ? rawVal : JSON.stringify(rawVal);
                if (valStr && valStr.length > 8 * 1024 * 1024) {
                    console.warn('[backup] 跳过超大数据键: ' + key + ' (' + Math.round(valStr.length/1024) + 'KB)');
                    continue;
                }
        ...
        if (typeof window !== 'undefined' && typeof window.compressImageBase64 === 'function') {
            ...
            // 主页背景、卡片背景、头像、app图标等大图 —— localStorage 版本
            var homeImgKeys = ['home_page_bg_custom', 'home_card_bg_custom', 'home_avatar_me', 'home_avatar_partner'];
            for (var hi = 0; hi < homeImgKeys.length; hi++) {
                var hkey = homeImgKeys[hi];
                // 同时处理带 session 前缀和不带前缀的两种 key 格式
                var allLsKeys = Object.keys(lsData).filter(function(k) { return k === hkey || k.indexOf('_' + hkey) !== -1; });
                for (var hki = 0; hki < allLsKeys.length; hki++) {
                    var hk = allLsKeys[hki];
                    if (lsData[hk] && typeof lsData[hk] === 'string' && lsData[hk].length > 150 * 1024 && lsData[hk].indexOf('data:image') === 0) {
                        try {
                            var hCompressed = await window.compressImageBase64(lsData[hk], { maxWidth: 1200, quality: 0.78 });
                            if (hCompressed && hCompressed.length < lsData[hk].length) {
                                console.log('[backup] 兜底压缩(ls) ' + hk + ': ' + lsData[hk].length + ' -> ' + hCompressed.length);
                                lsData[hk] = hCompressed;
                            }
                        } catch (hErr) { /* ignore */ }
                    }
                }
            }
            // 主页背景、卡片背景、头像等大图 —— localforage 版本（大图优先存 localforage）
            var lfImgPatterns = ['home_page_bg_custom', 'home_card_bg_custom', 'home_avatar_me', 'home_avatar_partner', 'callBgImageData'];
            var lfKeys2 = Object.keys(lfData);
            for (var lfi = 0; lfi < lfKeys2.length; lfi++) {
                var lfk = lfKeys2[lfi];
                var isImgKey = lfImgPatterns.some(function(pat) { return lfk === pat || lfk.indexOf('_' + pat) !== -1 || lfk.indexOf(pat) !== -1; });
                if (!isImgKey) continue;
                if (typeof lfData[lfk] === 'string' && lfData[lfk].length > 150 * 1024 && lfData[lfk].indexOf('data:image') === 0) {
                    try {
                        var lfCompressed = await window.compressImageBase64(lfData[lfk], { maxWidth: 1200, quality: 0.78 });
                        if (lfCompressed && lfCompressed.length < lfData[lfk].length) {
                            console.log('[backup] 兜底压缩(lf) ' + lfk + ': ' + lfData[lfk].length + ' -> ' + lfCompressed.length);
                            lfData[lfk] = lfCompressed;
                        }
                    } catch (lfErr) { /* ignore */ }
                }
            }
        ...
        for (var k in lsRaw) {
            ...
            try {
                ...
                if (targetLsKey === 'moments_data' || targetLsKey.indexOf('moments_data') !== -1) {
                    ...
                    // 主页大图（功能图标、背景、头像等）写 localStorage 失败时，降级写 localforage
                    // home.js 已有 localforage 兜底读取逻辑
                    var homeImgFallbackPatterns = ['home_app_icons', 'home_page_bg_custom', 'home_card_bg_custom',
                        'home_avatar_me', 'home_avatar_partner', 'profile_partner', 'profile_me'];
                    var isHomeLargeKey = homeImgFallbackPatterns.some(function(pat) {
                        return targetLsKey === pat || targetLsKey.indexOf(pat) !== -1;
                    });
                    if (isHomeLargeKey && typeof localforage !== 'undefined') {
                        try {
                            var lsv2 = lsv || processLocalStorageValueForImport(lsRaw[k], mediaStore);
                            // localStorage 存 JSON 字符串，localforage 也存 JSON 字符串保持一致
                            await localforage.setItem(targetLsKey, lsv2);
                            console.log('[backup] ' + targetLsKey + ' 已降级保存到 localforage');
                        } catch (e3) {
                            console.warn('[backup] home 数据 localforage 降级也失败', targetLsKey, e3);
                        }
                    } else {
